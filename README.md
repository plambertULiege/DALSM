...
where $f_{j}^{\mu}(\cdot)$ and $f_{j}^{\sigma}(\cdot)$ denote
smooth$$\log{\sigma(\mathbf{z}_i, \mathbf{x}_i)}= \delta_{0}+\sum_{k=1}^p \delta_k z_ {ik}+\sum_{j=1}^{J} {{f_{j}^{\sigma}}}(x_ {ij})$$
...
$$\mathbf{f}^\mu_j=\left(\sum_{\ell=1}^L {b_{j\ell}}(x_{ij})\theta_{\ell
    j}^\mu\right)_{i=1}^n=\mathbf{B}_j\pmb{\theta}_j^\mu~~;~~ 
\mathbf{f}^\sigma_j=\left(\sum_{\ell=1}^L {b_{j\ell}}(x_{ij})\theta_{\ell j}^\sigma\right)_{i=1}^n=\mathbf{B}_j\pmb{\theta}_j^\sigma$$
