...
Besides fitting Nonparametric Double Additive Location-Scale Model to censored data, the DALSM package contains an independent and very fast function, *densityIC*, for density estimation from right- or interval-censored data with possible constraints on the mean and variance.
...
head(xmat,15)
```
The density can be estimated from the censored data using function *densityIC*. Optionally, the mean and variance of the estimated density can also be forced to some fixed values, here 5.0 and 2.5, respectively. We also choose to force the left end of the distribution support to be 0:
```{r DALSM3c}
