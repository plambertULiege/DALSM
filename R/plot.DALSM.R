## GOAL:
##  Visualize the fitted additive terms in the location-scale model
##
## INPUT:
##  obj.fit: object generated by LocationScaleFit
##  mfrow.loc: desired window-plot division for additive terms in location
##  mfrow.disp: desired window-plot division for additive terms in dispersion
##  xx: values in (0,1) for the standardized covariates at which the additive terms must be plotted
...
## OUTPUT:
##  Plot with
##     The estimated standardized residuals and the fitted error density
##     The fitted additive terms for location and (log-)dispersion
##  Mean Coverage of point-wise confidence intervals for additive terms over the (0,1) range
##     returned as an invisible list of vectors with coverage.loc and coverage.disp as returned elements
##
## -----------------------------------
...
#'        nx=101, plot.global=TRUE, true.loc=NULL,true.disp=NULL, ci.level=NULL,
#'        error.lim = NULL, plot.residuals=FALSE, true.derr=NULL, ...)
#'
...
#' @param plot.global logical indicating if the same range of values must be used when plotting the fitted additive terms (default: TRUE)
...
#' @param plot.residuals logical requesting to add the (possibily censored) standardized residuals to the plot of the fitted standardized error density (default: FALSE)
#' @param true.derr (optional) density function to superpose to the estimated standardized error density (default: NULL)
...
#'
#' @return In addition to the plots, an invisible return of a list containing:
#' \itemize{
...
plot.DALSM = function(x,
  ...
  if (plot.residuals){
    plot.densHazardConstrainedFit(obj.fit$derr,histRC=TRUE,breaks=25,xlim=error.lim,
                                  xlab="Std.residuals",
  ...
  if (J1 > 0){
    ...
    for (j in 1:J1){
      ...
      x.loc[,j] = xx = seq(xlow,xup,length=nx)
      ...
      if (plot.global) ylims = c(min(cred.low),max(cred.up))
      else  ylims = c(min(fj.min),max(fj.max))
      plotRegion(xx,cbind(fj.hat,fj.min,fj.max),
                 xlab=xlab,ylab=colnames(f.loc)[j], ## paste("f1.",j,"(x)",sep=""),
                 ylim=ylims,
                 main=paste("Loc:  ",colnames(f.loc)[j],sep="")) ## main=paste("Loc:  f1.",j,"(x)",sep=""))
  ...
  if (J2 > 0){
    ...
    # colnames(x.disp) = paste("x.disp.",1:J2,sep="")
    # colnames(f.disp) = paste("f.disp.",1:J2,sep="")
    # colnames(se.disp) = paste("se.disp.",1:J2,sep="")
    for (j in 1:J2){
      ...
      plotRegion(xx,cbind(fj.hat,fj.min,fj.max),
                 xlab=xlab,ylab=colnames(f.disp)[j], ##paste("f2.",j,"(x)",sep=""),
                 ylim=ylims,
