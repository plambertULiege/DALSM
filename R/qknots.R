...
#' \itemize{
#' \item \code{xmin} : specified minimum value for the knots, except if \eqn{\min(x) < xmin}, in which case the default value \eqn{\min(x)-sd(x)} is returned.
#' \item \code{xmin} : specified maximum value for the knots, except if \eqn{xmax < \max(x)}, in which case the default value \eqn{\max(x)+sd(x)} is returned.
#' \item \code{K} : number of B-splines.
#' \item \code{knots} : equidistant knots on (xmin,xmax) if \code{equidistant.knots} is TRUE, based on quantiles of \code{x} otherwise.
#' \item \code{Pd} : \eqn{K\times K} penalty matrix of order \code{pen.order}.
#' \item \code{pen.order} : a reminder of the requested penalty order (Default: 3).
#' }
#' @export
#'
#' @examples
